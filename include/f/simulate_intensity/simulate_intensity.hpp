...
namespace f
{
    inline void simulate_intensity_c( matrix<unsigned long> const& ar, matrix<std::complex<double> > const& ug, matrix<double> const& diag, double const thickness, std::string const& output )
    {
        ...
        for ( unsigned long index = 0; index != diag.col(); ++index )
        {
            ...
            std::string fname = "/testpool/ops/samfairman/QEDA/bin/Examples/2020_01_15_SrTiO3_T0/_feng_input_zero_tilt/test.txt";
            S.save_as(fname.c_str());
    ...
    inline void simulate_intensity_with_tilt( char const* unit_cell, char const* const beam, char const* const ar, char const* const ug, char const* const tilt, double const thickness, char const* const output, double radius, double ev, unsigned long index = 0  )
    {
        std::cout<<"\nev = "<<ev<<", radius = "<<radius<<", index = "<<index<<"\n"<<std::endl;
        matrix<double> mbeam;
        mbeam.load( beam );
        assert( mbeam.row() );
        assert( mbeam.col() == 4 );
