...
namespace f
{
    ...
    void pattern_to_image( std::string const& beams_path, std::string const& tilts_path, std::string const& intensities_path, std::string const& image_path, double const rotation = 0.0 )
    {
        ...
        image.save_as_bmp( image_path, std::string{"bluered"}, std::string{"logx"} );
        //image.save_as_bmp( image_path, std::string{"jet"}, std::string{"log"} );
        //image.save_as_bmp( image_path, std::string{"jet"}, std::string{"logpi"} );
        //image.save_as_bmp( image_path, std::string{"jet"}, std::string{"log1"} );
        //image.save_as_bmp( image_path, std::string{"obscure"} );
        //image.save_as_bmp( image_path );
        //
        //image.save_as( image_path + std::string{".txt"} );

        image.save_as_txt( "./current_pattern.txt" );
