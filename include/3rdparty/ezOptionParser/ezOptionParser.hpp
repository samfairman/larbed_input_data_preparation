...
#define VALIDATE(T, U, LIST) { \
    ...
    void OptionGroup::getString( char* out )
    {
        std::string string_out;
        getString( string_out );
        std::copy( string_out.begin(), string_out.end(), out );
    }
    ...
    class ezOptionParser
    {
        public:
            ...
            inline void parse( int argc, char** );
    ...
    void ezOptionParser::resetArgs()
    {
        ...
        for ( i = 0; i < ( long int )groups.size(); ++i )
        {
            groups[i]->cleaOptionGroup::getDouble( double& out )
    {
        if ( !isSet )
        {
            if ( defaults.empty() )
    ...
    void ezOptionParser::parse( int argc, char** argv )
    {
        if ( argc < 1 )
        {
            return;
    ...
    void ezOptionParser::parse( int argc, const char* argv[] )
    {
        ...
        char** new_argv = const_cast<char**>( argv );
        parse( argc, new_argv );
    }
