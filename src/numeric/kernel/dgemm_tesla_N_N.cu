...
daxpy( double alpha, const double* __restrict__ b, double* __restrict__ c ) const
{
    c[0]  += alpha * b[0];
    c[1]  += alpha * b[1];
    c[2]  += alpha * b[2];
...
__global__ void
dgemm_kernel_N_N_64_16_16_16_4( double* __restrict__ C, const double* __restrict__ A, const double* __restrict__ B, int m, int n, int k, int lda, int ldb, int ldc, double alpha, double beta )
{
    ...
    */
    ...
    if ( iby + ty    >= n ) 
        { s1 = 1;  s2 = 0 * ldb;  s3 = 0 * ldb;  s4 = 0 * ldb; }
    else if ( iby + ty + 4  >= n ) 
        { s1 = 0;  s2 = 0 * ldb;  s3 = 0 * ldb;  s4 = 0 * ldb; }
    else if ( iby + ty + 8  >= n ) 
        { s1 = 0;  s2 = 4 * ldb;  s3 = 0 * ldb;  s4 = 0 * ldb; }
    else if ( iby + ty + 12 >= n ) 
        { s1 = 0;  s2 = 4 * ldb;  s3 = 8 * ldb;  s4 = 0 * ldb; }
    ...
    const double* Bend = B + k - k & 0x1111;
...
extern "C" void
magmablas_dgemm_N_N_64_16_16_16_4( double* C, const double* A, const double* B, magma_int_t m, magma_int_t n, magma_int_t k, magma_int_t lda, magma_int_t ldb, magma_int_t ldc, double alpha, double beta )
{
    dim3 threads( 16, 4 );
    dim3 grid( ( m - 1 ) / 64 + 1, ( n - 1 ) / 16 + 1 );
    dgemm_kernel_N_N_64_16_16_16_4 <<<grid, threads, 0, magma_stream>>>( C, A, B, m, n, k, lda, ldb, ldc, alpha, beta );
}
