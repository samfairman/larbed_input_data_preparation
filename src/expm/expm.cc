...
c_matrix const expm( const c_matrix& A )
{
    ...
    const unsigned long s = nm < 1.0 ? 0 : static_cast<unsigned long>( std::ceil( std::log( nm ) - std::log(2.0) ) );
    const float s_2 = s ? ( 1 << s ) : 1;
    std::cout<<"\nnorm of A 1"<<nm<<std::endl;
...
z_matrix const expm( const z_matrix& A )
{
    assert( A.row() == A.col() );
    const unsigned long n = A.row();
    const double nm = A.norm();
    //const unsigned long s = nm < 1.0 ? 0 : static_cast<unsigned long>( std::ceil( std::log2( nm ) ) );
    const unsigned long s = nm < 1.0 ? 0 : static_cast<unsigned long>( std::ceil( std::log( nm ) - std::log(2.0) ) );
    const double s_2 = s ? ( 1 << s ) : 1;
    std::cout<<"\nnorm of A 3"<<nm<<std::endl;
    const z_matrix& a = A / s_2; //the scaled matrix A

    z_matrix a_n = a; // keeps a^n
    z_matrix ans = eye_z_matrix(n); //the ans of e^A
    const unsigned long loops = 10;
    double factor = 1.0;
    for ( unsigned long i = 1; i != loops; ++i )
    {
    ...
    /*
       ...
       1.0257020561269          0.0641622038836256          0.00569162022190139         0.00728977857272476
       */
    //        const double c0[4] = { 0.963405714485326, 0.583581039167404, 0.144651158571306, 0.0134226317770268 };
    //        const double c1[4] = { 1.01197445118178, 0.335669463894947, 0.0857542043420956, 0.0121745404670141 };
    //        const double c2[4] = { 1.0257020561269, 0.0641622038836256, 0.00569162022190139, 0.00728977857272476 };
    //
    /*
    1.0476935979303396707
    0.13621658523122884277
    ...
    0.023963840848021274871
    */
    //const double c0[4] = { 1.0476935979303396707, 0.13621658523122884277, 0.17503285846386393287, 0.0001955881771752682294 };
    //const double c1[4] = { 0.91452063342352907238, 0.67099326513851953457, 0.17186329865997720945, 0.00019137545763237996514 };
    //const double c2[4] = { 1.043692545657082027, 0.13755748432914494117, -0.067225888057316357527, 0.023963840848021274871 };
    /*
    0.73769924933048902638
    0.13172525137117249705
    0.072080085802694884856
...
// M = e^A
extern "C"
void zexpm( const std::complex<double>* A, unsigned long n, std::complex<double>* M )
{
    double* a_ = new double[ 2 * n * n ];
    for ( unsigned long i = 0; i != n * n; ++i )
    {
        a_[i + i]     = A[i].real();
        a_[i + i + 1]   = A[i].imag();
    }
    const z_matrix a( n, n, a_ ); //the matrix A constructed from host memory
    z_matrix const& ans = expm( a );
    ans.export_to( a_ );
    for ( unsigned long i = 0; i != n * n; ++i )
        M[i] = std::complex<double>( a_[i + i], a_[i + i + 1] );
    delete[] a_;
}
