...
static __device__
void operator ()( double alpha, const double* __restrict__ b, double*       __restrict__ c ) const
{
    c[0]  += alpha * b[0];
    c[1]  += alpha * b[1];
    c[2]  += alpha * b[2];
...
dgemm_kernel_N_N_64_16_16_16_4( double*       __restrict__ C, const double* __restrict__ A, const double* __restrict__ B, int m, int n, int k, int lda, int ldb, int ldc, double alpha, double beta )
{
    ...
    */
    ...
    if ( iby + ty    >= n ) { s1 = 1;  s2 = 0 * ldb;  s3 = 0 * ldb;  s4 = 0 * ldb; }
    else if ( iby + ty + 4  >= n ) { s1 = 0;  s2 = 0 * ldb;  s3 = 0 * ldb;  s4 = 0 * ldb; }
    else if ( iby + ty + 8  >= n ) { s1 = 0;  s2 = 4 * ldb;  s3 = 0 * ldb;  s4 = 0 * ldb; }
    else if ( iby + ty + 12 >= n ) { s1 = 0;  s2 = 4 * ldb;  s3 = 8 * ldb;  s4 = 0 * ldb; }
    if ( s1 == 0 )
        B += __mul24( ty, ldb );
    else
        s1 = 0;
