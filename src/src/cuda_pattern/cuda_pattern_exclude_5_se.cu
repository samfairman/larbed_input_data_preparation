...
extract_intensity_diff_with_offset( double2* __restrict__  s, double* __restrict__  I_exp, double* __restrict__ I_diff, unsigned long dim, unsigned long column_index, double ac_offset, double dc_offset )
{
    ...
    //double const res = *(I_exp+I_offset) - norm * norm * ac_offset - dc_offset;
    double const res = *(I_exp+I_offset) - norm * norm * (1.0+ac_offset) - dc_offset;
    double const residual = sqrt( res*res+1.0e-10 );
    *(I_diff+I_offset) = residual;
}
