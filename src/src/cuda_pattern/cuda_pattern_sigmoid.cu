...
extract_intensity_diff_with_offset( double2* s, double* I_exp, double* I_diff, unsigned long dim, unsigned long column_index, double ac_offset, double dc_offset )
{
    ...
    *(I_diff+I_offset) = val - norm * norm * ac_offset - dc_offset;
    //*(I_diff+I_offset) *= 1.0 / ( 1.0 + exp( 2.0*3.1415926535897932384626433*val ) );
    *(I_diff+I_offset) *= 1.0 / ( 1.0 + exp( 12.56637061435917295384*val ) );
}
