...
__global__ void Dznrm2( unsigned long int n, double2* x, double* the_norm )
{
    ...
    void Zgemm( double2*__restrict__ P, double2* __restrict__  M, double2* __restrict__  N, unsigned long dim, double alpha )
    {
    ...
    __global__ void//<<<1, 128>>>
    Zaxpy( unsigned long dims, double real, double imag, double2* __restrict__ dst, double2* __restrict__  src ) // dst += (real,imag) * src
    {
        unsigned long const i = threadIdx.x;
        double R = 0.0;
        double I = 0.0;
...
__global__ void
compose_a( double* __restrict__  ug, unsigned long* __restrict__  ar, double* __restrict__  diag, double thickness, double2* __restrict__  a, unsigned long dim )
{
	thickness = 13.0 * ( 1.0+thickness*10.0 );
...
__global__ void
extract_intensity_diff( double2* __restrict__  s, double* __restrict__  I_exp, double* __restrict__  I_diff, unsigned long dim, unsigned long column_index )
{
...
__global__ void
extract_intensity_diff_with_offset( double2* __restrict__  s, double* __restrict__  I_exp, double* __restrict__ I_diff, unsigned long dim, unsigned long column_index, double ac_offset, double dc_offset )
{
    int const I_offset = threadIdx.x;
    int const S_offset = column_index + threadIdx.x * dim;
    double const norm = cuCabs(*(s+S_offset));
    ...
    double const df = sqrt(*(I_exp+I_offset)) - norm * (1.0+ac_offset*10.0) - dc_offset;
    *(I_diff+I_offset) = sqrt( df*df + 1.0e-10 );
}
 ...
 */
__global__ void
make_individual_pattern_intensity_diff( double* __restrict__  cuda_ug, unsigned long* __restrict__  cuda_ar, double* __restrict__  cuda_diag, double thickness, unsigned long* __restrict__  cuda_dim, double* __restrict__  cuda_I_exp, double* __restrict__  cuda_I_diff, unsigned long column_index, double2* __restrict__  cuda_cache, unsigned long max_dim, unsigned long tilt_size, double ac_offset, double dc_offset )
{
...
void make_pattern_intensity_diff( double* cuda_ug, unsigned long* cuda_ar, double* cuda_diag, double thickness, unsigned long* cuda_dim, double* cuda_I_exp, double* cuda_I_diff, unsigned long column_index, double2* cuda_cache, unsigned long tilt_size, unsigned long max_dim, double ac_offset, double dc_offset )
{
    unsigned long const threads = 128;
    unsigned long const grids = (tilt_size + threads - 1)/threads;

    kernel_assert( ( make_individual_pattern_intensity_diff<<<grids, threads>>>( cuda_ug, cuda_ar, cuda_diag, thickness, cuda_dim, cuda_I_exp, cuda_I_diff, column_index, cuda_cache, max_dim, tilt_size, ac_offset, dc_offset ) ) );
    cuda_assert( cudaDeviceSynchronize() );
}
