...
__global__ //Zgemm<<<((dim+15)/16,(dim+15)/16), (16,16)>>>(....)
 ...
 */
__global__ void
make_individual_pattern_intensity_diff( double* cuda_ug, unsigned long* cuda_ar, double* cuda_diag, double* cuda_thickness, unsigned long* cuda_dim, double* cuda_I_exp, double* cuda_I_diff, unsigned long column_index, double2* cuda_cache, unsigned long max_dim, unsigned long tilt_size )
{
    unsigned long const tilt_index = blockDim.x * blockIdx.x + threadIdx.x;

    if ( tilt_index >= tilt_size )
    {
        return;
    }
    ...
    double const thickness = cuda_thickness[tilt_index];
...
void make_xpattern_intensity_diff( double* cuda_ug, unsigned long* cuda_ar, double* cuda_diag, double* cuda_thickness, unsigned long* cuda_dim, double* cuda_I_exp, double* cuda_I_diff, unsigned long column_index, double2* cuda_cache, unsigned long tilt_size, unsigned long max_dim )
{
    unsigned long const threads = 64;
    unsigned long const grids = ( tilt_size + threads - 1 ) / threads;

    kernel_assert( ( make_individual_pattern_intensity_diff<<<grids, threads>>>( cuda_ug, cuda_ar, cuda_diag, cuda_thickness, cuda_dim, cuda_I_exp, cuda_I_diff, column_index, cuda_cache, max_dim, tilt_size ) ) );
    cuda_assert( cudaDeviceSynchronize() );
}
